...
import { Leaf, Loader2, ClipboardCheck, Mail, Lock } from "lucide-react";
import { toast } from "sonner";
import { useAuthentication } from "../../utils/Auth";

const CollectorLogin = () => {
  const { loginCollector } = useAuthentication();
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await loginCollector(email, password);
      if (result.success) {
        toast.success("Login Successful", { description: "Welcome to your field agent portal" });
      ...
      <div className="w-full max-w-sm rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
        <Link to="/" className="mb-6 flex items-center justify-center gap-2">
          ...
          <div>
            ...
            <span className="block text-[6px] uppercase tracking-widest text-emerald-400">Field Agent</span>
          </div>
        </Link>
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className="text-xl font-extrabold text-white">Field Agent Sign In</h1>
          <p className="text-xs text-slate-400">Register farmers and apply on their behalf</p>
        </div>

        <form onSubmit={onSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="flex items-center gap-1 text-xs font-bold text-slate-300">
              <Mail size={13} className="text-emerald-500" /> Agent Email
            </label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="agent@cooperative.org"
